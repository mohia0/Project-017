            ...
            <div className={cn("flex items-center gap-0.5 px-4 pt-3 pb-1 shrink-0 overflow-x-auto scrollbar-none",
                ...
                {STATUSES.map(s => {
                    ...
                    return (
                        ...
                        >
                            {s !== 'All' && cfg && (
                                <span className="w-1.5 h-1.5 rounded-full" style={{ background: isDark ? STATUS_DARK[s as SchedulerStatus].dot : cfg.dot }} />
                            )}
                            {s}
            ...
            <DeleteConfirmModal
                ...
                onConfirm={async () => {
                    if (deletingId === 'bulk') {
                        ...
                        setSelectedIds(new Set());
                        gooeyToast.error(`${ids.length} scheduler${ids.length !== 1 ? 's' : ''} deleted`);
                    } else if (deletingId) {
                        await deleteScheduler(deletingId);
                        gooeyToast.error('Scheduler deleted');
                    }
                    setDeletingId(null);
                }}
            />
